...
//  GHFollowers
//
//  Created by Alejandro Trejo on 04/08/21.
//
...
class FollowerListVC: GFDataLoadingVC {
    ...
    func addUserToFavorites(user: User) {
        ...
        PersistenceManager.updateWith(favorite: favorite, actionType: .add) {[weak self] error in
            ...
            guard let error = error else {
                self.presentGFAlertOnMainThread(title: "Usuario agregado", message: "El usuario se agregó a favoritos 🎉", buttonTitle: "Entendido")
                return
            }
            self.presentGFAlertOnMainThread(title: "Algo salio mal", message: error.rawValue, buttonTitle: "Entendido")
        }
    ...
    func getFollowers(username: String, page: Int) {
        ...
        NewtworkManager.shared.getFollowers(for: username, page: page) {[weak self] result in
            ...
            case .failure(let error):
                self.presentGFAlertOnMainThread(title: "Test", message: error.rawValue, buttonTitle: "Entendido")
            }
        }
    }
    ...
    func updateUI(with followers: [Follower]) {
        ...
        if self.followers.isEmpty {
            let message = "Este usuario no tiene seguidores. Siguelo 😃."
